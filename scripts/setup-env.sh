...
cp -n packages/mcp-bedrock-client/.env.default packages/mcp-bedrock-client/.env 2>/dev/null || true
...
echo "  - packages/mcp-bedrock-client/.env (for AWS credentials and Bedrock config)"
